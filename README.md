...
ensure primary keys of tables can be reliably used as foreign keys others. This becomes harder to enforce
...
It reduces the boilerplate of managing a *Polymorphic Assication* modeled as an idea called an *Exclusive
...
Continuing our example, the generator command would also produce a migration that looks like this:

```ruby
class CommentCommentableExclusiveArc < ActiveRecord::Migration[7.0]
  def change
    add_reference :comments, :post, foreign_key: true, index: true
    add_reference :comments, :comment, foreign_key: true, index: true
...
The database constraint ensures `ActiveRecord` validations can’t be bypassed to break the fabeled rule -
"There Can Only Be One™️". # TODO: partial indexes
